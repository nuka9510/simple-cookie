...
export default class Cookie {
  ...
  constructor(
    ...
    if (Util.empty(name)) { throw new Error(`'name'이 비어있습니다.`); }
   ...
   * `Domain=domain;`
   */
  get domain(): string { return Util.empty(this.#domain) ? '' : `Domain=${ this.#domain };`; }
   ...
   * `Expires=expires;`
   */
  get expires(): string { return Util.empty(this.#expires) ? '' : `Expires=${ this.#expires.toUTCString() };`; }
   ...
   * `HttpOnly;`
   */
  get httpOnly(): string { return (this.#httpOnly) ? 'HttpOnly;' : ''; }
   ...
   * `Max-Age=maxAge;`
   */
  get maxAge(): string { return Util.isNumber(this.#maxAge, true) ? `Max-Age=${ this.#maxAge };` : ''; }
   ...
   * `Partitioned;`
   */
  get partition(): string { return (this.#partition) ? 'Partitioned;' : ''; }
   ...
   * `Path=path;`
   */
  get path(): string { return Util.empty(this.#path) ? '' : `Path=${ this.#path };`; }
   ...
   * `SameSite=sameSite;`
   */
  get sameSite(): string { return Util.empty(this.#sameSite) ? '' : `SameSite=${ this.#sameSite };`; }
   ...
   * `Secure;`
   */
  get secure(): string { return (this.#secure) ? 'Secure;' : ''; }
  ...
  static setCookie(
    cookie: Cookie
  ): void { document.cookie = cookie.toString(); }
